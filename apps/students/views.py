    ...
    def get_context_data(self, **kwargs):
        ...
        steps = [
            ...
            },
            {
                ...
                'is_completed': student.user is not None,
                'url': reverse_lazy('students:account_info', kwargs={'student_id': student.pk}),
                'icon': 'bx bx-lock-alt'
            }
        ]
...
class StudentAccountInfoView(BaseView):
    """Manage student account credentials"""
    template_name = 'students/forms/account_info.html'
    permission_required = 'students.change_student'
    
    def get(self, request, student_id):
        student = get_object_or_404(Student.get_secure_queryset(request.user), pk=student_id)
        
        if not student.user:
            # Should not happen in new flow, but safe fallback
            messages.warning(request, "Student has no user account. One will be created upon saving password.")
            form = SetPasswordForm(user=None) # We'll create user on post if needed
        else:
            form = SetPasswordForm(user=student.user)
    ...
    def post(self, request, student_id):
        ...
        if student.user:
            ...
            # Create user if missing (legacy support or failure case)
            student.create_user_account()
            user = student.user
            messages.info(request, "User account created automatically.")
        ...
        if form.is_valid():
            ...
            # Send email confirmation of password change if desired? 
            # Or reliance on the initial create email + ongoing management
            # User wants "confirmation of when student create account is created"
            # We assume this view is the "Account Info" step. 
            
            messages.success(request, f"Password for {student.full_name} has been updated.")
            return redirect('students:student_onboarding', pk=student.pk)
