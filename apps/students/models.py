...
class Student(BaseModel):
    ...
    enrollment_date = models.DateField(default=timezone.localdate, verbose_name=_("Enrollment Date"))
   ...
   # UPDATE THESE FIELDS:
    ...
    passing_year = models.DateTimeField(
        null=True,
        blank=True,
        verbose_name=_("Passing Year")
    )
    tc_issue_date = models.DateField(
        null=True,
        blank=True,
        verbose_name=_("TC Issue Date")
    )
    # ==================== ACADEMIC TRACKING ====================
    ...
    def is_eligible_for_exams(self):
        ...
        # Assuming invoice has a student field
        latest_payment = Payment.objects.filter(
            invoice__student=self  # Through invoice
        ).order_by('-created_at').first()
        
        # OR if Payment has an invoice with student
        ...
        return (
            ...
            latest_payment and 
            latest_payment.status == "COMPLETED" and  # Use correct status
    ...
    def get_attendance_percentage(self, start_date=None, end_date=None):
        ...
        attendances = StudentAttendance.objects.filter(student=self)
    ...
    def get_academic_performance(self):
        ...
        # Consider only passed exams for GPA calculation
        ...
        total_grade_points = sum(result.grade_point for result in results if result.grade_point)
        total_exams = results.count()
        
        return {
            "gpa": float(total_grade_points / total_exams) if total_exams > 0 else 0.0,
            "total_subjects": total_exams, # Reporting exams as subjects for now as per previous logic structure
    ...
    def generate_institutional_email(self):
        ...
        if not self.institutional_email and self.admission_number:
            # Use correct related name accessing Domain model
            # Use correct related name for DomainMixin
    ...
    def create_user_account(self):
        """Create system user account for student and send credentials"""
        if not self.user:
            ...
            if self.personal_email:
                subject = f"Welcome to {self.tenant.name if hasattr(self.tenant, 'name') else 'Our School'} - Login Credentials"
                message = f"""
Dear {self.first_name},

Welcome to {self.tenant.name if hasattr(self.tenant, 'name') else 'Our School'}!

Your student account has been created successfully. You can now access the student portal using the following credentials:

Portal URL: [Your School Portal URL]
Username/Email: {self.personal_email}
Password: {password}

Please change your password after your first login.

Best regards,
{self.tenant.name if hasattr(self.tenant, 'name') else 'School Administration'}
"""
                try:
                    send_mail(
                        subject=subject,
                        ...
                        recipient_list=[self.personal_email],
                        fail_silently=False
                    )
                except Exception as e:
                    # Log error but don't fail the transaction
                    print(f"Failed to send welcome email to {self.personal_email}: {e}")
    ...
    def clean(self):
        ...
        if self.date_of_birth:
             ...
             # Age validation - only check if DOB is present and valid
             try:
                 if self.age < 3:
        ...
        try:
            ...
            # Tenant might not be set during initial validation or tests
            pass
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.reg_no:
             self.reg_no = self.generate_reg_no()
    ...
    def generate_roll_number(self):
        """Generate incremental roll number based on class/section"""
        # Logic: Get max roll number for the same academic year + class + section (if exists)
        # Note: This is a simple incremental strategy.
        filters = {
            'tenant': self.tenant,
            'academic_year': self.academic_year,
        ...
        last_student = Student.objects.filter(**filters).exclude(roll_number='').order_by('roll_number').last()
    ...
    def generate_reg_no(self):
        """Generate unique registration number: REG-{YYYY}-{SEQ}"""
        ...
        last_student = Student.objects.filter(
            ...
            tenant=self.tenant
        ).order_by('reg_no').last()
        
        if last_student:
             try:
                last_seq = int(last_student.reg_no.split('-')[-1])
                new_seq = last_seq + 1
             except ValueError:
                new_seq = 1
        else:
...
class StudentDocument(BaseModel):
    ...
    def save(self, *args, **kwargs):
        ...
        # Handle versioning
        if self.pk and not self._state.adding:  # Existing document
