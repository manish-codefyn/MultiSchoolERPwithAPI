...
class GlobalSearchView(BaseTemplateView):
    """
    ...
    def get_context_data(self, **kwargs):
        ...
        print(f"DEBUG: User: {self.request.user}, Role: {self.request.user.role}")
        print(f"DEBUG: Tenant: {self.tenant}")

        if query:
            # Search Students
            # Check permission - simple check for now, can be refined
            if self.request.user.role in [User.ROLE_ADMIN, User.ROLE_SUPER_ADMIN, User.ROLE_PRINCIPAL, User.ROLE_VICE_PRINCIPAL, User.ROLE_TEACHER, User.ROLE_STAFF]:
                print("DEBUG: Searching Students...")
                try:
                    from apps.students.models import Student
            ...
            # Search Staff
            if self.request.user.role in [User.ROLE_ADMIN, User.ROLE_SUPER_ADMIN, User.ROLE_PRINCIPAL, User.ROLE_VICE_PRINCIPAL, User.ROLE_HR]:
                try:
                    from apps.hr.models import Staff
