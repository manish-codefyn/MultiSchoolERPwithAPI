...
from apps.academics.models import (
    ...
    ClassSubject, TimeTable, StudentAttendance, Holiday, StudyMaterial,
...
class Command(BaseCommand):
    ...
    def populate_for_tenant(self, tenant, options):
        ...
        with tenant_context(tenant):
            try:
                ...
                self.create_StudentAttendance_records(options['students_per_class'], classes, sections_by_class, options)
    ...
    def clear_tenant_data(self):
        ...
        models_to_clear = [
            ...
            Holiday, StudentAttendance, TimeTable, ClassSubject,
    ...
    def create_StudentAttendance_records(self, students_per_class, classes, sections_by_class, options):
        """Create StudentAttendance records"""
        self.stdout.write("Creating StudentAttendance records...")
        
        # Get or create students
        students = self.get_or_create_students(students_per_class, classes, sections_by_class, options)
        
        if not students:
            self.stdout.write(self.style.WARNING("  No students found. Skipping StudentAttendance creation."))
            return
        
        teachers = User.objects.filter(role='teacher') if not options['skip_teachers'] else []
        StudentAttendance_statuses = ['PRESENT', 'ABSENT', 'LATE', 'HALF_DAY', 'LEAVE']
        
        StudentAttendance_created = 0
        
        # Create StudentAttendance for last 3 days (reduced for performance)
        for i in range(3):
            ...
            for student in students[:10]:  # Limit to first 10 students for performance
                # Random StudentAttendance status with 85% present rate
                ...
                try:
                    ...
                    StudentAttendance_created += 1
                except Exception as e:
                    self.stdout.write(self.style.WARNING(f"    Error creating StudentAttendance record: {e}"))
        
        self.stdout.write(f"  Created {StudentAttendance_created} StudentAttendance records")
