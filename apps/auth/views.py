    ...
    def get_context_data(self, **kwargs):
        ...
        try:
            ...
            if is_public:
                ...
                total_tenants = 1
                active_tenants = 1 if current_tenant and current_tenant.is_active else 0
                suspended_tenants = 0
        ...
        # User Stats
        if is_public:
            # Bypass UserManager to see ALL users across system
            # Note: Assuming User is a shared model
            total_users = User._base_manager.count()
            recent_users = User._base_manager.order_by('-date_joined')[:5]
            role_qs = User._base_manager.all()
        else:
            # Filter for current tenant only
            total_users = User.objects.filter(tenant=current_tenant).count()
            recent_users = User.objects.filter(tenant=current_tenant).order_by('-date_joined')[:5]
            role_qs = User.objects.filter(tenant=current_tenant)
        
        # User Role Distribution
        role_distribution = role_qs.values('role').annotate(count=Count('id')).order_by('-count')
        role_labels = []
        role_data = []
